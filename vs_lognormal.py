...
x = np.random.lognormal(0,2,10000)*1e6
x = np.sort(x)
print(x)
y = crtCDF(x)
norm_x = preprocessing.scale(x)    # 標準化: 零均值化

plt.title("The CDF of Lognormal Distribution")
...
model.fit(norm_x, y, epochs=100, batch_size=32, verbose=0)  # norm_x:訓練資料, y:訓練目標
